/* This solution is based on Kahn's Algorithm

The algorithm implements a topological sort with three steps:-
...
We will take a count variable to check if there is a cycle in the graph
(because if there is a cycle in the graph, then it means that we cant apply topological sorting and there will be no way to complete all the courses).

So, in order to state the graph is a DAG (directed acyclic graph), our count variable should be equal to the total number of nodes in the graph.
